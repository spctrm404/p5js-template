class Responsive {
  ...
  createResponsiveCanvas(
    ...
    if (
      ...
      console.error(
        `@${functionName}(): 3번째 매개변수 canvasFit은 "contain", "cover", "none", "scale-down" 중 하나여야 합니다.`
      );
      return;
    }
    if (typeof staticCoordinate !== 'boolean') {
      console.error(
        `@${functionName}(): 4번째 매개변수 staticCoordinate는 true 또는 false여야 합니다.`
      );
      return;
    }
    ...
    if (!container) {
      console.error(
        `@${functionName}(): "${this.containerSelector}"와 일치하는 HTML 요소가 없습니다.`
      );
      return;
    }
  ...
  drawReferenceGrid(
    ...
    if (
      !this.#isColor(boundaryColour) ||
      !this.#isColor(gridColour) ||
      !this.#isColor(centerColour)
    ) {
      console.error(
